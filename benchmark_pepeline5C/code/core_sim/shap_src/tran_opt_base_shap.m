...
function [tr]=tran_opt_base_shap(tr)

if(tr.Nvec(end)>1) % 是否有延长时间,对稳态计算是没有的
...
if(tr.m.use_init_state==1)
    st_ind=0;
    pp_init=tr.ss_start.xf(st_ind+1:st_ind+FN); st_ind=st_ind+FN;
    qq_init=tr.ss_start.xf(st_ind+1:st_ind+NE); st_ind=st_ind+NE;
    cc_init=tr.ss_start.xf(st_ind+1:st_ind+C); st_ind=st_ind+C;
    st_ind=0;
    % pp_terminal=tr.ss_terminal.xf(st_ind+1:st_ind+FN); st_ind=st_ind+FN;
    % qq_terminal=tr.ss_terminal.xf(st_ind+1:st_ind+NE); st_ind=st_ind+NE;
    % cc_terminal=tr.ss_terminal.xf(st_ind+1:st_ind+C); st_ind=st_ind+C;
end
...
for v=1:length(tr.Nvec)        %iterate optimizations for increasing discretization
    ...
    if(tr.m.N==0) % static opt
        tr.m.d=tr.m.Yq1; % 稀疏阵：气体流出或进入的每个节点的流量，归一化后
        tr.m.dlb=[tr.m.Ylbd1; tr.m.Ylbs1]; % supply or demand 流量下界（实际数据对时间做了平均后）
        tr.m.dub=[tr.m.Yubd1; tr.m.Yubs1]; % supply or demand 流量上界
        tr.m.prd=[tr.m.Prd1; -tr.m.Prs1]; % supply or demand 价格
    ...
    if(v==1) % 第一次迭代优化 3步
        ...
        elseif(tr.m.use_init_state==1)
            ...
            % 在开头并上最初的init ss opt
            % pp0=[pp_init;pp0];qq0=[qq_init;qq0];cc0=[cc_init;cc0];
    ...
    if(tr.m.use_init_state==0)  %variables on time points 0 to N
        ...
        ubd=reshape(tr.m.dub,GN*N1,1);   % 时间的平均值作为约束               %on flexible demands 
        ...
        ipopt_options.cl = [tr.m.Beq;zeros(M*N1,1);dischpliml-1000000;hpliml-1000000];   %lower bounds on the constraint functions.
        if(tr.intervals>1 && v==length(tr.Nvec) && tr.m.N>0)
        ...
        ipopt_options.cu = [tr.m.Beq;zeros(M*N1,1);dischplimu+1000000;hplimu+1000000];   %upper bounds on the constraint functions.
        if(tr.intervals>1 && v==length(tr.Nvec) && tr.m.N>0)
    ...
    elseif(tr.m.use_init_state==1) %variables on time points 1 to N
        ...
        ipopt_options.cl = [tr.m.Beq;zeros(M*N,1);dischpliml-1000000;hpliml-100000000];   %lower bounds on the constraint functions.

        %upper bounds on constraints
        dischplimu=zeros(C*N,1);                                        %on discharge
        hplimu=zeros(C*N,1);                                              %on power
        ipopt_options.cu = [tr.m.Beq;zeros(M*N,1);dischplimu+1000000;hplimu+100000000];   %upper bounds on the constraint functions.
    ...
    % 当进行完第一步优化后，数组成为多个，因此即进行下一步动态优化，此处将迭代处理并保存所有时间的优化结果
    if(v>1)
        N1s=tr.m.N1s; % all-opt
        Ns=tr.m.Ns; % int opt
        if(tr.m.use_init_state==0)
            ...
            xu0=[reshape(pn',FN*N1,1);reshape(qn',NE*N1,1);reshape(cn',C*N1,1)];
    ...
    % IPOPT parameters
    tr.m.state_init=tr.ss_start.xf;
    % tr.m.state_init=tr.ss_start.xf;
    %scaling parameters
...
elseif(tr.m.use_init_state==1)
    %restriction to original horizon if extension used
    tr.pp0=[pp_init reshape(xf(1:N*FN),FN,N) xf(1:FN)];
    tr.qq0=[qq_init reshape(xf(N*FN+1:N*M),NE,N) xf(N*FN+1:N*FN+NE)];
    tr.cc0=[cc_init reshape(xf(N*M+1:N*M+N*C),C,N) xf(N*M+1:N*M+C) ];
