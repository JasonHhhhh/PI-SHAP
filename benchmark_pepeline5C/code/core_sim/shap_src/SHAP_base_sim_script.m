for k=[5,8,10,12,15]
    ...
    % res_baseline.par_ssopt=par_ssopt;
    % save('shap_src\res_baseline.mat','res_baseline')
    ...
    % end
    % save(['shap_src\data_all_shap_' num2str(k) '.mat'],'data_all_shap')
    load(['shap_src\data_all_shap_' num2str(k) '.mat'])
    ...
    % 提取std并实施
    for i=1:num_sampls
        shap_cost_every(i,:,:)=data_all_shap{i}.shap.item_shap_cost(:,:);
        shap_supp_every(i,:,:)=data_all_shap{i}.shap.item_shap_supp(:,:);
        shap_var_every(i,:,:)=data_all_shap{i}.shap.item_shap_var(:,:);
    end
    mean_cost=squeeze(mean(shap_cost_every,1));
    mean_supp=squeeze(mean(shap_supp_every,1));
    mean_var=squeeze(mean(shap_var_every,1));
    std_cost=squeeze(std(shap_cost_every,0,1));
    std_supp=squeeze(std(shap_supp_every,0,1));
    std_var=squeeze(std(shap_var_every,0,1));
    save(['shap_src\datastd_all_shap_' num2str(k) '.mat'],...
        'mean_cost','mean_supp','mean_var',...
        'std_cost','std_supp','std_var')
    for i=1:num_sampls
    ...
    shap_cost_label8 = cell(num_sampls,24);
    shap_supp_label8 = cell(num_sampls,24);
    shap_var_label8 = cell(num_sampls,24);
    for i=1:num_sampls
        for j=1:24
            if (shap_cost_norm(i,j)>=0) && (shap_cost_norm(i,j)<0.2)
                shap_cost_label8{i,j}='1';
            elseif (shap_cost_norm(i,j)>=0.2) && (shap_cost_norm(i,j)<0.4)
                shap_cost_label8{i,j}='2';
            elseif (shap_cost_norm(i,j)>=0.4) && (shap_cost_norm(i,j)<0.6)
                shap_cost_label8{i,j}='3';
            elseif (shap_cost_norm(i,j)>=0.6)
                shap_cost_label8{i,j}='4';
            elseif (shap_cost_norm(i,j)>=-0.2) && (shap_cost_norm(i,j)<0)
                shap_cost_label8{i,j}='-1';
            elseif (shap_cost_norm(i,j)>=-0.4) && (shap_cost_norm(i,j)<-0.2)
                shap_cost_label8{i,j}='-2';
            elseif (shap_cost_norm(i,j)>=-0.6) && (shap_cost_norm(i,j)<-0.4)
                shap_cost_label8{i,j}='-3';
            elseif (shap_cost_norm(i,j)<-0.6)
                shap_cost_label8{i,j}='-4';
            end
    ...
    for i=1:num_sampls
        for j=1:24
            if (shap_supp_norm(i,j)>=0) && (shap_supp_norm(i,j)<0.2)
                shap_supp_label8{i,j}='1';
            elseif (shap_supp_norm(i,j)>=0.2) && (shap_supp_norm(i,j)<0.4)
                shap_supp_label8{i,j}='2';
            elseif (shap_supp_norm(i,j)>=0.4) && (shap_supp_norm(i,j)<0.6)
                shap_supp_label8{i,j}='3';
            elseif (shap_supp_norm(i,j)>=0.6)
                shap_supp_label8{i,j}='4';
            elseif (shap_supp_norm(i,j)>=-0.2) && (shap_supp_norm(i,j)<0)
                shap_supp_label8{i,j}='-1';
            elseif (shap_supp_norm(i,j)>=-0.4) && (shap_supp_norm(i,j)<-0.2)
                shap_supp_label8{i,j}='-2';
            elseif (shap_supp_norm(i,j)>=-0.6) && (shap_supp_norm(i,j)<-0.4)
                shap_supp_label8{i,j}='-3';
            elseif (shap_supp_norm(i,j)<-0.6)
                shap_supp_label8{i,j}='-4';
            end
    ...
    for i=1:num_sampls
        for j=1:24
            if (shap_var_norm(i,j)>=0) && (shap_var_norm(i,j)<0.2)
                shap_var_label8{i,j}='1';
            elseif (shap_var_norm(i,j)>=0.2) && (shap_var_norm(i,j)<0.4)
                shap_var_label8{i,j}='2';
            elseif (shap_var_norm(i,j)>=0.4) && (shap_var_norm(i,j)<0.6)
                shap_var_label8{i,j}='3';
            elseif (shap_var_norm(i,j)>=0.6)
                shap_var_label8{i,j}='4';
            elseif (shap_var_norm(i,j)>=-0.2) && (shap_var_norm(i,j)<0)
                shap_var_label8{i,j}='-1';
            elseif (shap_var_norm(i,j)>=-0.4) && (shap_var_norm(i,j)<-0.2)
                shap_var_label8{i,j}='-2';
            elseif (shap_var_norm(i,j)>=-0.6) && (shap_var_norm(i,j)<-0.4)
                shap_var_label8{i,j}='-3';
            elseif (shap_var_norm(i,j)<-0.6)
                shap_var_label8{i,j}='-4';
            end
    ...
    label8={shap_cost_label8,shap_supp_label8,shap_var_label8};

    %% 建立tree; 通过tree筛选&控制策略
    % label_all =label8;
    ...
    %     n_leaf=zeros(24,1);
    %     for i=1:24
    %         X=squeeze(cat(3,shap_cc(:,i,:),shap_dcc(:,i,:)));
    %         Y=labels(:,i);
    %         % %     CV搜索最优叶子结点个数
    ...
    %         error(i)=min_error;
    %         disp(['saving' 'shap_src\data\ctree_' num2str(i) '_m' num2str(v) '_ls' num2str(k) '.mat........'])
    %         save(['shap_src\data\ctree_' num2str(i) '_m' num2str(v) '_ls' num2str(k) '.mat'],'tree')
    %     end
    % end
    % 
    % % how to use tree to get decision fastly!!! ------ scoring and ranking!
    % % 此时是model free得了；直接MC采样然后通过tree的rule-scoring ranking；取top策略即可，具体是:
    % % 每一个时刻都评价，针对每一个指标都单独评价；然后所有时间点求和
    % 
    % scores_all=[];
    % scores_multiX_all=[];
    % for j=1:num_sampls
    ...
    %             load(['shap_src\data\ctree_' num2str(i) '_m' num2str(v) '_ls' num2str(k) '.mat'],'tree')
    %             sc_this=predict(tree, [x_this; dx_this]');
    %             scores_multiX(v)=scores_multiX(v)+str2num(sc_this{:})*mean(x_this);
    %             scores(v)=scores(v)+str2num(sc_this{:});
    %         end
    %     end
    %     scores_all=[scores_all; scores];scores_multiX_all=[scores_multiX_all; scores_multiX];
    % end
    % save(['shap_src\data\scores_all' num2str(k) '_train.mat'],'scores_all')
    % save(['shap_src\data\scores_multiX_all' num2str(k) '_train.mat'],'scores_multiX_all')

    load(['shap_src\data\scores_all' num2str(k) '_train.mat'])
    load(['shap_src\data\scores_multiX_all' num2str(k) '_train.mat'])
    %% 训练集
    ...
    % 设置保存目录
    saveDir = ['shap_src\train_K' num2str(k)]; % 修改为你要保存的路径
    ...
    for i = 1:length(allFigs)
        ...
        if ~isempty(fig.Name)
            ...
            figName = ['UnnamedFigure_', num2str(fig.Number)];
        end

        % 去除文件名中的非法字符
        cleanName = regexprep(figName, '[^\w]', '_'); % 将非字母数字字符替换为下划线

        % 构造保存路径
        savePath = fullfile(saveDir, [cleanName, '.png']); % 可以修改格式如 .fig/.jpg

        % 激活当前图形窗口
        figure(fig);

        % 保存图形（根据需求选择保存方式）
        % saveas(fig, savePath); % 基本保存
        print(fig, savePath, '-dpng', '-r300'); % 更高分辨率（推荐）
