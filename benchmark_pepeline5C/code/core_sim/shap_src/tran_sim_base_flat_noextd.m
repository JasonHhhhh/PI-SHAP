function [sim]=tran_sim_base_flat_noextd(sim)
...
%startup simulation
t_int=[0 sim.c.T*sim.startup]; % 额外的启动时间
c_int=[sim.m.ccc0'; sim.m.cc0(:,1)']; % 启动时刻的压比，正式0时刻的压比，进行插值，启动时刻的压比是稳态优化来的
s_int=[sim.m.Pslack1'; sim.m.Ys(:,1)']; % 气源压力
d_int=[sim.m.Yd1';sim.m.Yd(:,1)']; % 出口流量
...
tic,[tt0,ppnd0,qqnd0]=pipe_sim_net_noextd(sim.m); sim.init_time=toc;
...
tic,[tt1,ppnd1,qqnd1]=pipe_sim_net_noextd(sim.m); sim.cycles_time=toc;
...
% tic,[tt2,ppnd2,qqnd2]=pipe_sim_net_noextd(sim.m); sim.cycles_time=toc;
% disp(['finished in ' num2str(sim.cycles_time) ' sec.'])
...
% 这里将最后一个时刻的加回来
sim.tt=[tt0(1:end-1)-sim.c.T*sim.startup;tt1(1:end)]; 
sim.ppnd=[ppnd0(1:end-1,:);ppnd1(1:end,:)];
sim.qqnd=[qqnd0(1:end-1,:);qqnd1(1:end,:)];
sim.cc=[cft0(1:end-1,:);cft1(1:end,:)];
numd=sim.n0.nv-sum(sim.n0.isslack);
sim.dd=[dft0(1:end-1,1:numd);dft1(1:end,1:numd)]*sim.c.qsc;
