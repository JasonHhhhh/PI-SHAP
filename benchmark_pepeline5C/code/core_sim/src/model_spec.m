...
par.c.b1=b1; par.c.b2=b2;
% else par.c.a=sqrt(par.c.gasR*par.c.gasT); end
par.c.a=sqrt(par.c.gasR*par.c.gasT/(b1+b2*mean([par.n.p_min;par.n.p_max])));

Z=1/(b1+b2*mean([par.n.p_min;par.n.p_max]));
...
% par.c.poly_index=1/((1+1/(par.c.gamm-1))*par.c.poly_eff-1)+1;
% par.c.T_Pr_lnRatio = par.c.poly_index/(par.c.poly_index-1);
% Zsc=(par.c.b1+par.c.b2*par.n.p_min(par.n.comp_pos(:,1)))./(par.c.b1+par.c.b2*par.n.p_max(par.n.comp_pos(:,1)));

% par.m.eff=par.m.fuelfactor./((par.n.c_max.*Zsc).^par.m.mpow-1); 	%fuel factor

%par.m.boost_pow_max_nd=(1./par.m.eff).*par.n.hp_max/par.c.mmscfd_to_hp*par.c.mmscfd_to_kgps/par.c.qsc;
par.m.Wc=par.c.gasR*par.c.gasT./par.c.mpow;
% par.m.Wc=par.c.gasR*par.c.gasT*(par.c.gamm/(par.c.gamm-1));
