...
Ts=par.Ts; w=par.w; D=-par.D*2/Ts; prslack=par.prslack;
...
%economic objective
cecon=supp_sign * sum((diag(xs(slinks))*prslack).*fs,1)*w;

%output objective
cecon_o=(Ts/2)*cecon/par.objsc;
ceff_o=(Ts/2)*ceff/par.objsc;
c=(Ts/2)*((ceff/cost_scale)*(1-ew)+(cecon/supp_scale)*ew)/par.objsc;
