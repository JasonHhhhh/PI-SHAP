...
if(par.out.plotmarketflowlims==1)
    ...
    plot(out.td,out.dbase(:,out.guniqueind).*par.ss.c.mmscfd_to_kgps,'LineWidth',3), axis('tight'), xlabel('hours')
    if(par.out.units==1), title('Baseline Gas Withdrawals (kg/s)','fontweight','bold'), else
    ...
    if(~isempty(par.tr.dmax_pos))
        subaxis(1,3,2,'SpacingHoriz',0.05), 
        plot(out.td,out.gdub,'LineWidth',3), axis('tight'), xlabel('hours'), hold on,
        plot(out.td,out.gdlb,'--','LineWidth',3), hold off, legend(num2str(par.tr.dmax_pos),'Location','SouthEast')
    ...
    if(~isempty(par.tr.smax_pos))
        subaxis(1,3,3,'MarginRight',0.05,'SpacingHoriz',0.05), 
        plot(out.td,out.gsub,'LineWidth',3), axis('tight'), xlabel('hours'), hold on,
        plot(out.td,out.gslb,'--','LineWidth',3), hold off, legend(num2str(par.tr.smax_pos),'Location','SouthEast')
...
if(par.out.plotmarketpricebids==1)
    ...
    if(~isempty(par.tr.smax_pos))
        ...
        legend(num2str(par.tr.smax_pos),'Location','SouthEast')
    end
    if(par.out.plotpdf==1)
    set(gcf,'PaperPositionMode', 'manual','PaperUnits','points', ...
        'Paperposition',paperpos), set(gcf, 'PaperSize', papersize)
...
if(par.out.plotcompshadow==1)
    ...
    subaxis(1,2,1,'MarginLeft',0.05,'SpacingHoriz',0.05), 
    plot(out.tt0,out.mult0_pmax.*(abs(out.mult0_pmax)>10^(-3)),'LineWidth',3), axis('tight'), xlabel('hours')
    legend(num2str([1:out.n0.nc]'),'Location','SouthEast'), 
    ...
    subaxis(1,2,2,'MarginRight',0.05,'SpacingHoriz',0.05),
    plot(out.tt0,out.mult0_cmax.*(abs(out.mult0_cmax)>10^(-3)),'LineWidth',3), axis('tight'), xlabel('hours')
