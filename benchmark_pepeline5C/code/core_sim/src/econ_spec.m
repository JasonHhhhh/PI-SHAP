...
mfolder = strrep(mfolder, '\\', filesep);
mfolder = strrep(mfolder, '/', filesep);
readCsv = @(name) double(dlmread(fullfile(mfolder, name)));

if(par.intervals>0)
    intervalFiles = {'input_int_qbar.csv', 'input_int_gbar.csv', 'input_int_dmax.csv', ...
        'input_int_smax.csv', 'input_int_cd.csv', 'input_int_cs.csv', ...
        'input_int_cslack.csv', 'input_int_pslack.csv'};
    hasIntervalFiles = true;
    for fileId = 1:length(intervalFiles)
        if(exist(fullfile(mfolder, intervalFiles{fileId}), 'file')~=2)
            hasIntervalFiles = false;
            break;
        end
    end
    if(~hasIntervalFiles)
        warning('Interval input files are missing in %s. Falling back to input_ts_*.csv.', mfolder);
        par.intervals=0;
    end
end
...
if(par.intervals==0)
    ...
    xd=readCsv('input_ts_tpts.csv'); % 时间索引
    qbar=readCsv('input_ts_qbar.csv'); % 通过用户分输量得到的出口流量 8个 其余都是0 但是所有非气源节点都要有29
    gbar=readCsv('input_ts_gbar.csv'); % 所有用户分输流量 15个，对应节点求和则得到qbar,有几个分输节点就写几个
    dmax_0=readCsv('input_ts_dmax.csv'); % 需求负荷边界
    smax_0=readCsv('input_ts_smax.csv'); % 供气负荷边界
    cd_0=readCsv('input_ts_cd.csv'); % 需求用气的价格
    cs_0=readCsv('input_ts_cs.csv'); % 供货价格
    cslack=readCsv('input_ts_cslack.csv'); % 气源供气价格
    pslack=readCsv('input_ts_pslack.csv'); % 气源压力
...
if(par.intervals>0)
    ...
    int_qbar0=readCsv('input_int_qbar.csv');
    int_gbar=readCsv('input_int_gbar.csv');
    int_dmax0=readCsv('input_int_dmax.csv');
    int_smax0=readCsv('input_int_smax.csv');
    int_cd0=readCsv('input_int_cd.csv');
    int_cs0=readCsv('input_int_cs.csv');
    int_cslack=readCsv('input_int_cslack.csv');
    int_pslack=readCsv('input_int_pslack.csv');
...
%     par.m.state_init=double(dlmread([mfolder '\input_state-init.csv'])); 
%     par.m.state_terminal=double(dlmread([mfolder '\input_state-terminal.csv'])); 
% end
