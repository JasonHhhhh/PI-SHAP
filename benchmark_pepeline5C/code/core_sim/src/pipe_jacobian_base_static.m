function [J]=pipe_jacobian_base_static(x,par)
...
d(par.guniqueind,:)=d(par.guniqueind,:);
qcomp=q(clinks,:);

if(par.doZ==1), b1=par.b1; b2=par.b2; psc=par.psc; end

J=sparse(N1*M,N1*(M+C));        %allocate space
%define indices
...
elseif(par.use_init_state==1)
    ...
    Jineq2=Jineq2+sparse(II,JJ,SS,N*C,N1*(M+C));
    end
    J=[J;Jineq2*par.hplsc];
end
