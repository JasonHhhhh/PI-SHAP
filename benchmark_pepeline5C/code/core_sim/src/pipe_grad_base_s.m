...
%index parameters
N1=par.N1; N=par.N; M=par.M; C=par.C; FN=par.FN; NE=par.NE; GN=par.GN;GN=0;
Ts=par.Ts; w=par.w; D=-par.D*2/Ts; prd=par.prd; prslack=par.prslack;
clinks=par.comp_pos(:,2); slinks=par.comp_pos(par.spos,2);
%problem parameters
m=par.mpow; xsc=par.xs(clinks); xss=par.xs(slinks); ew=par.econweight;
...
for j=1:length(slinks)
    %dedfs
    cxecon(N1*FN+slinks(j):NE:N1*FN+NE*(N1-1)+slinks(j))=(supp_sign * xss(j) * prslack(j,:)').*w;
end
% for j=1:GN
%     %dedfd
%     cxecon(N1*(M+C)+j:GN:N1*(M+C)+GN*(N1-1)+j)=-prd(j,:)'.*w;
% end
cx_eff_block = (cxeff + cxsmc) / cost_scale;
cx_econ_block = cxecon / supp_scale;
cx=(Ts/2)*(cx_eff_block*(1-ew) + cx_econ_block*ew)/par.objsc;
