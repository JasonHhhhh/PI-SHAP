...
inputParamXls = fullfile(par.mfolder, 'input_param.xls');
inputParamCsv = fullfile(par.mfolder, 'input_param.csv');
if(exist(inputParamXls, 'file')==2)
    [inputpar]=xlsread(inputParamXls);  %use if xls
else
    inputpar_0=importdata(inputParamCsv);
...
par.tr.output_file=fullfile(par.mfolder, 'print_log_ss.txt');
par.ss.output_file=fullfile(par.mfolder, 'print_log_tr.txt');

%optimization options
par.tr.optintervals=inputpar(2,4);
if(inputpar(1)==0), par.tr.Nvec=0; 
end
if(inputpar(1)==1), par.tr.Nvec=0;
end
if(inputpar(1)>1)
...
par.tr.c.poly_eff=0.85;
par.tr.c.mpow=(par.tr.c.gamm-1)/par.tr.c.gamm/par.tr.c.poly_eff;
...
if(par.out.intervals>0)
    ...
    for fileId = 1:length(intervalFiles)
        if(exist(fullfile(par.mfolder, intervalFiles{fileId}), 'file')~=2)
            hasIntervalFiles = false;
            break;
        end
    end
    if(hasIntervalFiles)
        par.tr.optintervals=par.out.intervals;
        par.out.intervals_out=par.out.intervals;
    else
        warning('Interval input files are missing in %s. Falling back to input_ts_*.csv.', par.mfolder);
        par.out.intervals=0;
        par.tr.intervals=0;
        par.ss.intervals=0;
        par.tr.m.intervals=0;
        par.tr.optintervals=0;
        par.out.intervals_out=0;
    end
end
...
%state save points
stateSavePtsFile = fullfile(par.mfolder, 'input_state_save_pts.csv');
if(exist(stateSavePtsFile, 'file')==2)
    par.tr.m.save_state=1; par.ss.m.save_state=1;
    par.tr.m.state_save_pts=double(dlmread(stateSavePtsFile));
elseif(exist(stateSavePtsFile, 'file')==0)
